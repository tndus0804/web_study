...
public class BoardServiceImpl implements BoardService {
	...
	public void write(BoardDTO boardDTO, String uploadPath, MultipartFile upload) throws IOException {
		MemberEntity memberEntity = 
			...
			.orElseThrow(
				() -> new EntityNotFoundException("아이디가 없습니다.")
			);
	...
	public List<BoardDTO> getListAll() {
		// boardNum 을 정렬기준으로 데이터를 entity로 가져오기
	...
	/**
	 * DB에서 조회한 게시글 정보인 BoardEnity객체를 BoardDTO 객체로 변환
	 * @param entity 객체
	 * @return dto 객체
	 */
		...
		try {
			/*
				...
				2. URLEncoder.encode()
				  ...
				  클라이언트에서 처리할때 문제가 발생할 수 있으니 이 메서드를 통해
	...
	public void update(BoardDTO boardDTO, String username
		...
		BoardEntity boardEntity = 
			boardRepository.findById(boardDTO.getBoardNum())
				   .orElseThrow(
	...
	public void replyWrite(ReplyDTO replyDTO) {
		...
		MemberEntity memberEntity = 
			memberRepository.findById(replyDTO.getMemberId())
			.orElseThrow(() -> new EntityNotFoundException("사용자 아이디가 없습니다."));
		// 글정보를 DB로부터 조회 및 가져오기
		BoardEntity boardEntity =
			boardRepository.findById(replyDTO.getBoardNum())
			.orElseThrow(() -> new EntityNotFoundException("게시글이 없습니다."));
